...
def predict(data,*args,**kwargs)->np.float32:
    ...
    tol = np.array(data['tolerance'], dtype=np.float32)#точность расчетов
    m = np.array(data['acceptable'], dtype=np.float32)  #допустимое число отказов в год на 1 км длины
    z = np.array(data['nelements'], dtype=np.float32)  # число элементов трубопровода на на 1 км длины (по умолчанию 100)
    q = np.array(data['proba'], dtype=np.float32)  # доверительная вероятность
    ...
    ds = np.array(data['sigma0'], dtype=np.float32)  # относительное среднее квадратичное начальное
    # отклонение толщины стенки трубы в начальный момент,
    # равное 0,04 для бесшовных труб и 0,03 – для сварных труб;
    s0 = np.array(data['s0'], dtype=np.float32) #отбраковочная толщина.
    t = np.array(data['t'], dtype=np.float32) #возраст трубопровода на момент последней диагностики.
    tfirst=np.array(data['tfirst'], dtype=np.float32) #возраст перой аварии по коррозии.
